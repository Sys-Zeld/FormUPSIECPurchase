...
function validateProfileFields(profileFields) {
  ...
  if (!profileFields.length) {
    errors.fieldIds = "Select at least one field.";
    return errors;
  }
  const enabledCount = profileFields.filter((f) => f.isEnabled).length;
  if (!enabledCount) {
    errors.fieldIds = "Select at least one enabled field.";
  }
  ...
  for (const field of profileFields) {
    await client.query(
      `
        ...
        VALUES
          ($1, $2, $3, $4, $5, $6, $7, $8::jsonb, $9, $10::jsonb, $11, NOW())
      `,
      [
        profileId,
        field.fieldId,
